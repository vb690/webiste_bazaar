...
This section define the code used for updating the parameters associated with each of the arms, this function will be called dynamically during the simulation and will update the arms specified by the `policy` selected by the algoirthm with the `reward` associated with said policy.
...
Here we define all the functionality required for simulating the interactions with the various arms as well as the process of receiveing rewards, executing the policies and updating the parameters. There is quite a bit to un-pack here so we will proceed step by step.

Also in here, whenever is possible we will try to rely on the auto-vectorization capacity provided by `jax`.
